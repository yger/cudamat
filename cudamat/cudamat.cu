...
EXPORT int allocate_device_memory_sparse(cudamat_sparse* mat) {
...
EXPORT int copy_sparse_to_device(cudamat_sparse* mat) {
...
EXPORT void init_from_sparse_array(cudamat_sparse* mat, float* data, int* indices, int* indptr, int m, int n, int nnz) {
...
EXPORT int sparse_dot(cudamat_sparse* mat1, cudamat* mat2, cudamat* target, float beta, float alpha) {
    if (!mat1->on_device || !mat2->on_device || !target->on_device)
        return ERROR_NOT_ON_DEVICE;
    int m = mat1->size[0],
        k = mat1->size[1],
